...
"""
Created on Mon March  2 07:15:57 2020
@author: annapustova
"""
    ...
    for epoch in range(1, epochs + 1):
        for batch_index, (questions_in_batch, responses_in_batch) in enumerate(get_mini_batch(tr_questions, tr_responses, mini_batch)):
            ...
            with tf.GradientTape() as tape: # compute outputs
                ...
                output = tf.reshape(output, [-1, vocab_size]) # compute perplexity and optimize
...
def begin_dialogue(model, word_embed, embed_to_word, checkpoint="./chatbot2_weights.npz", top_n=TOP_N):
    ...
    while(True):
        ...
        for w_id in predicted_response[0]:
            ...
            if word == '<EOS>':
                break
        print('OES bi-RNN-LSTM: ' + ' '.join(response))


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--train', action='store_true', help='train model')
    parser.add_argument('--dataset-name', default='KK', type=str,
                        choices=['OES', 'KK'], help='dataset name')
    parser.add_argument('--dataset-path', type=str,
                        default='oes-bi-RNN-LSTM_data.json', help='file path for the OES data')
    parser.add_argument('--checkpoint', type=str, default='cornell_chatbot_weights.npz',
    ...
    with tf.device(device):
        vocab_size, word_embed, embed_to_word, sorted_prepro_responses, sorted_prepro_questions = prepro_sets(dataset_name,
                                                                                                          dataset_dir)
        model = Seq2seq(
            ...
            embedding_layer=tl.layers.Embedding(
                vocabulary_size=vocab_size, embedding_size=embed_size),
        )

        if args.train:
            optimizer = tf.optimizers.Adam(learning_rate=learn_rate)
            train_model(model, optimizer, sorted_prepro_responses=sorted_prepro_responses,
                        sorted_prepro_questions=sorted_prepro_questions, word_embed=word_embed, vocab_size=vocab_size, dataset_name=dataset_name)
            checkpoint = dataset_name + '_chatbot_weights.npz'

        begin_dialogue(model, word_embed=word_embed, embed_to_word=embed_to_word,
